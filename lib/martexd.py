...
class MarteXDaemon():
    def __init__(self, **kwargs):
        ...
        # memoize calls to some martexd methods
    ...
    @classmethod
    def from_martex_conf(self, marte_dot_conf):
        from martex_config import MarteXConfig
        config_text = MarteXConfig.slurp_config_file(marte_dot_conf)
        creds = MarteXConfig.get_rpc_creds(config_text, config.network)
    ...
    def get_my_gobject_votes(self, object_hash):
        ...
        if not self.gobject_votes.get(object_hash):
            ...
            self.gobject_votes[object_hash] = martexlib.parse_raw_votes(raw_votes)
    ...
    def we_are_the_winner(self):
        ...
        winner = martexlib.elect_mn(block_hash=current_block_hash, mnlist=mn_list)
        my_vin = self.get_current_masternode_vin()
    ...
    def estimate_block_time(self, height):
        ...
        future_seconds = martexlib.blocks_to_seconds(diff)
    ...
    def has_sentinel_ping(self):
        ...
        return (getinfo['protocolversion'] >= config.min_martexd_proto_version_with_sentinel_ping)
