...
sentinel_cfg = MarteXConfig.tokenize(sentinel_config_file)
sentinel_version = "1.3.0"
min_martexd_proto_version_with_sentinel_ping = 70207


def get_martex_conf():
    if sys.platform == 'win32':
        martex_conf = os.path.join(os.getenv('APPDATA'), "MXT/MarteX.conf")
    else:
        home = os.environ.get('HOME')

        martex_conf = os.path.join(home, ".MXT/MarteX.conf")
        if sys.platform == 'darwin':
            martex_conf = os.path.join(home, "Library/Application Support/MXT/MarteX.conf")

    martex_conf = sentinel_cfg.get('martex_conf', martex_conf)
