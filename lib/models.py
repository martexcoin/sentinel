...
class GovernanceObject(BaseModel):
    ...
    @classmethod
    def sync(self, martexd):
        golist = martexd.rpc_command('gobject', 'list')
        ...
        for item in golist.values():
            try:
                (go, subobj) = self.import_gobject_from_martexd(martexd, item)
            except Exception as e:
    ...
    @classmethod
    def import_gobject_from_martexd(self, martexd, rec):
        import decimal
        import martexlib
        import binascii
        import gobject_json
        ...
        # exclude any invalid model data from martexd...
        valid_keys = subclass.serialisable_fields()
        subdikt = {k: dikt[k] for k in valid_keys if k in dikt}

        # get/create, then sync vote counts from martexd, with every run
        ...
        # get/create, then sync payment amounts, etc. from martexd - MarteXd is the master
        try:
            newdikt = subdikt.copy()
            newdikt['object_hash'] = object_hash
            if subclass(**newdikt).is_valid() is False:
                govobj.vote_delete(martexd)
                return (govobj, None)

            subobj, created = subclass.get_or_create(object_hash=object_hash, defaults=subdikt)
        except Exception as e:
            # in this case, vote as delete, and log the vote in the DB
            printdbg("Got invalid object from martexd! %s" % e)
            govobj.vote_delete(martexd)
            return (govobj, None)
    ...
    def vote_delete(self, martexd):
        if not self.voted_on(signal=VoteSignals.delete, outcome=VoteOutcomes.yes):
            self.vote(martexd, VoteSignals.delete, VoteOutcomes.yes)
        return
    ...
    def vote(self, martexd, signal, outcome):
        import martexlib
        ...
        output = martexd.rpc_command(*vote_command)

        # extract vote output parsing to external lib
        voted = martexlib.did_we_vote(output)
        ...
        if voted:
            ...
            self.sync_network_vote(martexd, signal)

    def sync_network_vote(self, martexd, signal):
        printdbg('\tSyncing network vote for object %s with signal %s' % (self.object_hash, signal.name))
        vote_info = martexd.get_my_gobject_votes(self.object_hash)
...
class Proposal(GovernanceClass, BaseModel):
    ...
    govobj_type = MARTEXD_GOVOBJ_TYPES['proposal']
    ...
    def is_valid(self):
        ...
        try:
            ...
            # payment address is valid base58 marte addr, non-multisig
            if not martexlib.is_valid_marte_address(self.payment_address, config.network):
                printdbg("\tPayment address [%s] not a valid MarteX address for network [%s], returning False" % (self.payment_address, config.network))
                return False
            ...
            # MarteX Core restricts proposals to 512 bytes max
    ...
    def is_expired(self, superblockcycle=None):
        ...
        expiration_window_seconds = int(
            (martexlib.blocks_to_seconds(superblockcycle) / 2) +
...
class Superblock(BaseModel, GovernanceClass):
    ...
    govobj_type = MARTEXD_GOVOBJ_TYPES['superblock']
    ...
    def is_valid(self):
        ...
        for addr in addresses:
            if not martexlib.is_valid_marte_address(addr, config.network):
    ...
    def hash(self):
        import martexlib
        return martexlib.hashit(self.serialise())
