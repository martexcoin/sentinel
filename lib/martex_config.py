...
class MarteXConfig():
    ...
    def slurp_config_file(self, filename):
        # read marte.conf config but skip commented lines
        f = io.open(filename)
        lines = []
        for line in f:
        ...
        # data is marte.conf without commented lines
        data = ''.join(lines)
    ...
    def get_rpc_creds(self, data, network='mainnet'):
        # get rpc info from marte.conf
        match = re.findall(r'rpc(user|password|port)=(.*?)$', data, re.MULTILINE)

        # python >= 2.7
        creds = {key: value for (key, value) in match}

        # standard MarteX defaults...
        default_port = 51314 if (network == 'mainnet') else 41314

        # use default port for network if not specified in marte.conf
        if not ('port' in creds):
            creds[u'port'] = default_port

        # convert to an int if taken from marte.conf
