...
def is_valid_marte_address(address, network='mainnet'):
    ...
    martex_version = 111 if network == 'testnet' else 50
    ...
    try:
        ...
        # rescue from exception, not a valid MarteX address
        return False

    if (address_version != martex_version):
        return False
...
def did_we_vote(output):
    ...
    try:
        detail = output.get('detail').get('MarteX.conf')
