...
def has_martex_conf():
    ...
    valid_martex_conf = False

    # ensure martex_conf exists & readable
    #
    # if not, print a message stating that MarteX Core must be installed and
    # configured, including JSONRPC access in marte.conf
    try:
        f = io.open(config.martex_conf)
        valid_martex_conf = True
    except IOError as e:
        print(e)

    return valid_martex_conf
...
def main():
    ...
    if not has_martex_conf():
        print("MarteX Core must be installed and configured, including JSONRPC access in marte.conf")
        sys.exit(1)
